...
export async function getUserData(
    ...
    const response = await octokit.graphql<
        GraphQlQueryResponseData & { rateLimit: RateLimitInfo }
    >(
...
export async function getRepoData(
    ...
    logRateLimit(
        (response as { rateLimit?: RateLimitInfo }).rateLimit,
        "getRepoData",
    );
            ...
            viewer {
              contributionsCollection(from: $from, to: $to) {
                ...
                .then((response) => {
                    logRateLimit(
                        response.rateLimit,
                        `getContributionCollection year ${i}`,
                    );
                    return response;
                })
                .catch((error) => {
...
export async function getReposContributorsStats(
    ...
    | Awaited<ReturnType<typeof octokit.rest.repos.getContributorsStats>>
    | undefined
> {
    try {
        ...
        if (response.status === 202) {
            if (retryCount >= MAX_RETRY_COUNT) {
                console.warn(
                    `Max retries (${MAX_RETRY_COUNT}) reached for ${owner}/${repo} after exponential backoff, skipping`,
                );
                return undefined;
            }
            ...
            if (retryCount === 0) {
                console.log(
                    `[202] ${owner}/${repo}: GitHub computing stats, waiting ${actualDelay}ms...`,
                );
            } else {
                console.log(
                    `[202] ${owner}/${repo}: Retry ${retryCount + 1}/${MAX_RETRY_COUNT}, waiting ${actualDelay}ms...`,
                );
            }

            await new Promise((resolve) => setTimeout(resolve, actualDelay));
            return getReposContributorsStats(
                octokit,
                owner,
                repo,
                retryCount + 1,
        ...
        console.error(
            `Error fetching contributor stats for ${owner}/${repo}:`,
            error,
        );
        return undefined;
    }
...
export function calculateContributionStats(
    ...
    for (const week of contributionsCollection.contributionCalendar.weeks) {
        for (const day of week.contributionDays) {
            ...
            monthlyMap.set(
                month,
                (monthlyMap.get(month) || 0) + day.contributionCount,
            );
    ...
    const monthlyBreakdown: MonthlyContribution[] = Array.from(
        monthlyMap.entries(),
    )
...
export function aggregateLanguages(
    ...
    const languageMap = new Map<
        string,
        { color: string | null; value: number }
    >();
    let codeByteTotal = 0;
...
export function calculateComputedStats(
    ...
    const activeRepos = repoInfoList.filter((r) =>
        r.updatedAt.startsWith(currentYearStr),
    ).length;
    ...
    const primaryLanguageThisYear =
        topLanguagesThisYear[0]?.languageName || null;
    ...
    const yearOverYearGrowth =
        contributionsLastYear > 0
            ? Math.round(
                  ((contributionsThisYear - contributionsLastYear) /
    ...
    let mostProductiveMonth: { month: string; contributions: number } | null =
        null;
    for (const m of contributionStats.monthlyBreakdown) {
        if (
            !mostProductiveMonth ||
            m.contributions > mostProductiveMonth.contributions
        ) {
...
async function main() {
    ...
    const repoInfoList: RepoInfo[] = repos.map(
        (repo: {
            name: string;
    ...
    console.log(
        `All repo stats fetch time: ${(contribStats2 - contribStats1).toFixed(2)}ms`,
    );
    ...
    console.log(
        `Parse contributor stats time: ${(parseStats2 - parseStats1).toFixed(2)}ms`,
    );
    ...
    console.log(
        `Parse views time: ${(parseViews2 - parseViews1).toFixed(2)}ms`,
    );
    ...
    console.log(
        `Parse languages time: ${(parseLang2 - parseLang1).toFixed(2)}ms`,
    );
    ...
    console.log(
        `Calculate contribution stats time: ${(calcStats2 - calcStats1).toFixed(2)}ms`,
    );
    ...
    const tableData = [
        ...
        [
            ...
            computedStats.primaryLanguageThisYear || "N/A",
        ],
        ...
        [
            "YoY Growth",
            computedStats.yearOverYearGrowth !== null
                ? `${computedStats.yearOverYearGrowth}%`
                : "N/A",
        ],
        [
            "Total Contributions",
    ...
    const output = {
        ...
        repositoriesContributedTo:
            userData.user.repositoriesContributedTo.totalCount,
        discussionsStarted: userData.user.repositoryDiscussions.totalCount,
        discussionsAnswered:
            userData.user.repositoryDiscussionComments.totalCount,
        totalContributions:
            contributionsCollection.contributionCalendar.totalContributions,
        contributionStats,
        repoStats,
